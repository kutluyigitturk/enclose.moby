...
function openFeedback() {
    forceCloseMenu();
    window.open('https://docs.google.com/forms/d/e/1FAIpQLSd539q8PzCO6biD75NaEutSgp1rP7MSpT_zwrzgyGRMkxr30Q/viewform', '_blank');
}
...
function updateLevelStats() {
    ...
    const authorHtml = lvl.author === 'enclose.mobyAi'
        ? `<b class="author-ai-toggle">${lvl.author} <span class="ai-badge">AI</span></b>`
        : `<b>${lvl.author}</b>`;

    const aiExplanation = lvl.author === 'enclose.mobyAi'
        ? `<div class="ai-explain" id="ai-explain">
            <span>${t('aiAuthorTip')}</span>
            <a href="https://github.com/kutluyigitturk/enclose.mobyAi" target="_blank" class="ai-github-link">
    ...
    container.innerHTML = `
        ...
        <div class="stat-line" style="color:#333;"><span>${t('statMadeBy')}    </span>   ${authorHtml}</div>
        ${aiExplanation}
    ...
    const toggle = container.querySelector('.author-ai-toggle');
    if (toggle) {
        toggle.onclick = () => {
